$(function(){
	...
	$(window).scroll(function(){ //对页面进行滚动监听
		...
		if(getScrollTop()<200){
			...
			$(".fix_m").css("top","100px");
			// 公共尾部 导航栏移动
			if(getScrollTop()>$(".pinpai").offset().top+$(".pinpai").height()-window.screen.availHeight){
				var diff_down = getScrollTop()-$(".pinpai").offset().top-$(".pinpai").height()+window.screen.availHeight;
		...
		for(var x = 0; x < contentArr.length ; x++){
			if(contentArr[x].offsetTop<getScrollTop() && contentArr[x].offsetTop + contentArr[x].clientHeight >= getScrollTop()){
